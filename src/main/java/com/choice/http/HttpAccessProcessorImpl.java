...
import com.choice.utils.ErrorPageHelper;
import com.choice.utils.MIMEHelper;
import com.choice.utils.ServletMappingHelper;
...
public class  HttpAccessProcessorImpl implements HttpAccessProcessor  {
	...
	public void processStaticResource(HttpRequest request,HttpResponse response)
	{
		...
		response.setContentType(MIMEHelper.getMimeValue(contentType));
		response.setCRLF();
	...
	public void processDynamicResource(HttpRequest request,HttpResponse response)
	{
		...
		try {
			String path= ServletMappingHelper.getMappingValue(request.getRequestPath());
	...
	public void sendError(int statusCode,HttpRequest request,HttpResponse response)
	{
		  System.out.println("进入sendError()");
			...
			response.printResponseContent(ErrorPageHelper.getErrorPage(statusCode+""));
